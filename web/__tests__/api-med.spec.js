...
const build404 = () => Promise.resolve({ ok: false, status: 404, json: async () => ({}) });
  ...
  jest.spyOn(global, 'fetch').mockImplementation((url) =>
    url.startsWith('https://api.upcitemdb.com')
      ? buildOk({
          code: 'OK',
          total: 1,
          items: [{ title: 'Tylenol Extra Strength', images: ['https://example.com/t.png'] }],
        })
      : build404()
  );
...
test('falls back to DailyMed v1 if UPC gives no results', async () => {
  ...
  jest.spyOn(global, 'fetch').mockImplementation((url) => {
    if (url.startsWith('https://api.upcitemdb.com')) return buildOk({ code: 'OK', total: 0, items: [] });
    if (url.startsWith('https://dailymed.nlm.nih.gov/dailymed/services/v1')) return buildOk(dm1Json);
    return build404();
  });
...
test('uses DailyMed v2 hard-coded fallback when everything else 404s', async () => {
  ...
  jest.spyOn(global, 'fetch').mockImplementation((url) => {
    ...
    if (url.startsWith('https://dailymed.nlm.nih.gov/dailymed/services/v1')) return build404();
